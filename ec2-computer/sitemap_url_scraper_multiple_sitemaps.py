...
# run file: python sitemap_url_scraper_ice.py "29" "[2016,1,1,2017,1,19]" "https://www.ice.gov" "ice_urls.csv" "ice_403_postpose.csv" "ice_all_urls.csv"
if __name__ == '__main__':
#def main():
    iter_sitemaps = sys.argv[1]
    dates = sys.argv[2].replace('[', ' ').replace(']', ' ').replace(',', ' ').split() # dates = [2016,1,1,2017,1,19]
    dates = [int(i) for i in dates]
    domain = sys.argv[3] # url = https://www.nih.gov/; domain = "https://www.hhs.gov"
    output_file = sys.argv[4] # output_file = 'hhs_urls.csv'
    postpone_file = sys.argv[5] # postpone_file = "ice_403_postpose.csv"
    excluded_file = sys.argv[6] # excluded_file = "ice_all_urls.csv"

    # sitemaps to search
    endings = []
    for k in range(1,int(iter_sitemaps)+1):
        endings.append("?page=" + str(k))

    headers = {'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/42.0.2311.135 Safari/537.36 Edge/12.246'}

    for ending in endings:
        # acquire sitemap information
        r = requests.get(domain + "/sitemap.xml" + ending, headers=headers) 
        xml_text = r.text
    ...
    with open(sys_output_file,'w') as output: # append the site to the crawled url
